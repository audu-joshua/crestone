  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex justify-between w-full py-3 items-center">
        ...
        >
          <div
            className={`flex py-2 px-4 gap-2 rounded-2xl items-center ${styles.menuBgColor} ${styles.menuTextColor} transition-all duration-300 ease-in-out ${hamburger ? "justify-between w-full" : "justify-between"}`}
          >
    ...
    >
      <div className="flex flex-col gap-2 p-4">
        {menuItems.map((item, index) => (
          ...
          >
            {item.text}
            <FaLongArrowAltRight className={`${hoveredIndex === index ? "text-white" : ""}`} />
          </Link>
        ))}
      </div>
    </div>
